...
public class AddAnnouncementTest {
    ...
    @After
    public void cleanUp() {
        userController.getUser(user.getID(), object -> {
            ArrayList<String> signedUpEvents = object.getSignedUpEvents();
            ArrayList<String> organizedEvents = object.getOrganizedEvents();
    ...
    /**
     * Tests to see if Announcement input written but cancelled instead of sent-> doesn't add to signed-up Announcements
   ...
   Espresso.onView(ViewMatchers.withId(R.id.user_home_organizing_btn))
     .perform(ViewActions.click());
       ...
       try {
        ...
        Espresso.onView(ViewMatchers.withText("Attendee Sign-Ups"))
                .perform(scrollTo());
